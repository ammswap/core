...
using DummyREWARD as rewardToken
...
methods {
	...
	poolInfoAccRewardPerShare(uint256 pid) returns (uint128) envfree
	...
	// REWARD token
	REWARD() returns (address) envfree

	// Rewarder
	onTokenReward(uint256, address, address, uint256, uint256) => NONDET
...
rule monotonicityOfAccRewardPerShare(uint256 pid, method f) {
	env e;

	uint128 _poolInfoAccRewardPerShare = poolInfoAccRewardPerShare(pid);

	calldataarg args;
	f(e, args);

	uint128 poolInfoAccRewardPerShare_ = poolInfoAccRewardPerShare(pid);

	assert compareUint128(poolInfoAccRewardPerShare_, _poolInfoAccRewardPerShare);
}
...
rule noChangeToOtherPool(uint256 pid, uint256 otherPid) {
	...
	uint128 _otherAccRewardPerShare = poolInfoAccRewardPerShare(otherPid);
	...
	uint128 otherAccRewardPerShare_ = poolInfoAccRewardPerShare(otherPid);
	uint64 otherLastRewardBlock_ = poolInfoLastRewardBlock(otherPid);
	uint64 otherAllocPoint_ = poolInfoAllocPoint(otherPid);

	assert(_otherAccRewardPerShare == otherAccRewardPerShare_, "accRewardPerShare changed");
...
rule preserveTotalAssetOfUser(method f, uint256 pid, address user,
	...
	require REWARD() != lpToken(pid); // <-- check this again (Nurit)
...
rule rewardGivenInHarvestEqualsPendingReward(uint256 pid, address user, address to) {
	env e;

	require to == user && user != currentContract && e.msg.sender == user;
	require rewardToken == REWARD();

	uint256 userRewardBalance = rewardToken.balanceOf(e, user);
	uint256 userPendingReward = pendingReward(e, pid, user);

	// Does success return value matters? Check with Nurit
	harvest(e, pid, to);

	uint256 userRewardBalance_ = rewardToken.balanceOf(e, user);

	assert(userRewardBalance_ == (userRewardBalance + userPendingReward),
		   "pending reward not equal to the reward given in harvest");
}
...
function callFunctionWithParams(method f, uint256 pid, address sender, address to) {
	...
	} else if (f.selector == pendingReward(uint256, address).selector) {
		pendingReward(e, pid, to);
