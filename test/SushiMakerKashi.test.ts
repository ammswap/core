...
describe("KashiRewardMaker", function () {
  ...
  beforeEach(async function () {
    ...
    await deploy(this, [["exploiter", this.RewardMakerKashiExploitMock, [this.kashiMaker.address]]])
    ...
    await this.reward.approve(this.bento.address, getBigNumber(10))
    ...
    await this.bento.deposit(this.reward.address, this.alice.address, this.alice.address, getBigNumber(10), 0)
    ...
    //const initData = defaultAbiCoder.encode(["address", "address", "address", "bytes"], [this.reward.address, this.dai.address, this.oracle.address, oracleData])
    //await this.bento.deploy(this.KashiMaster.address, initData, true)
  })

  describe("setBridge", function () {
    it("only allows the owner to set bridge", async function () {
      await expect(this.kashiMaker.connect(this.bob).setBridge(this.reward.address, this.weth.address, { from: this.bob.address })).to.be.revertedWith("Ownable: caller is not the owner")
    })
    
    it("does not allow to set bridge for Reward", async function () {
      await expect(this.kashiMaker.setBridge(this.reward.address, this.weth.address)).to.be.revertedWith("Maker: Invalid bridge")
    })

    it("does not allow to set bridge for WETH", async function () {
      await expect(this.kashiMaker.setBridge(this.weth.address, this.reward.address)).to.be.revertedWith("Maker: Invalid bridge")
    })
    ...
    it("emits correct event on bridge", async function () {
      await expect(this.kashiMaker.setBridge(this.dai.address, this.reward.address))
  ...
  describe("convert", function () {
    it("reverts if caller is not EOA", async function () {
      await expect(this.exploiter.convert(this.reward.address)).to.be.revertedWith("Maker: Must use EOA")
    })
  })
})
