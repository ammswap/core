...
describe("Timelock", function () {
  ...
  it("should not allow non-owner to do operation", async function () {
    await this.reward.transferOwnership(this.timelock.address)
    // await expectRevert(this.reward.transferOwnership(carol, { from: alice }), "Ownable: caller is not the owner")

    await expect(this.reward.transferOwnership(this.carol.address)).to.be.revertedWith("Ownable: caller is not the owner")
    await expect(this.reward.connect(this.bob).transferOwnership(this.carol.address)).to.be.revertedWith("Ownable: caller is not the owner")
  ...
  it("should do the timelock thing", async function () {
    ...
    await this.timelock
      .connect(this.bob)
      .queueTransaction(this.reward.address, "0", "transferOwnership(address)", encodeParameters(["address"], [this.carol.address]), eta)
    await increase(duration.days(1))
    await expect(
      this.timelock
        .connect(this.bob)
        .executeTransaction(this.reward.address, "0", "transferOwnership(address)", encodeParameters(["address"], [this.carol.address]), eta)
    ).to.be.revertedWith("Timelock::executeTransaction: Transaction hasn't surpassed time lock.")
    await increase(duration.days(4))
    await this.timelock
      .connect(this.bob)
      .executeTransaction(this.reward.address, "0", "transferOwnership(address)", encodeParameters(["address"], [this.carol.address]), eta)
    expect(await this.reward.owner()).to.equal(this.carol.address)
  })
