...
describe("MasterChefV2", function () {
  before(async function () {
    await prepare(this, ["MasterChef", "SwivelToken", "ERC20Mock", "MasterChefV2", "RewarderMock", "RewarderBrokenMock"])
  ...
  beforeEach(async function () {
    ...
    await deploy(this, [
      ...
      ["chef", this.MasterChef, [this.reward.address, this.bob.address, getBigNumber(100), "0", "0"]],
    ])
    ...
    await deploy(this, [
      ["chef2", this.MasterChefV2, [this.chef.address, this.reward.address, 1]],
  ...
  describe("PendingReward", function () {
    it("PendingReward should equal ExpectedReward", async function () {
      ...
      await advanceBlock()
      let expectedReward = getBigNumber(100)
        .mul(log2.blockNumber + 1 - log.blockNumber)
        .div(2)
      let pendingReward = await this.chef2.pendingReward(0, this.alice.address)
    ...
    it("Should emit event LogUpdatePool", async function () {
      ...
      await expect(this.chef2.updatePool(0))
        ...
        .withArgs(
          ...
          (await this.chef2.poolInfo(0)).accRewardPerShare
        )
    })
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of REWARD and reward", async function () {
      ...
      let expectedReward = getBigNumber(100)
        .mul(log2.blockNumber - log.blockNumber)
        .div(2)
      expect((await this.chef2.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-" + expectedReward)
      await this.chef2.harvest(0, this.alice.address)
      expect(await this.reward.balanceOf(this.alice.address))
    ...
    it("Harvest for REWARD-only pool", async function () {
      ...
      expect((await this.chef2.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-" + expectedReward)
      await this.chef2.harvest(0, this.alice.address)
      expect(await this.reward.balanceOf(this.alice.address)).to.be.equal(expectedReward)
    })
  })
